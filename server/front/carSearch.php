...
<section class="hero-wrap hero-wrap-2 js-fullheight" style="background-image: url('assets/images/bg_3.jpg');" data-stellar-background-ratio="0.5">
	...
	if(isset($_GET['search'])){
		$inputField = $_GET['search'];
		$sqlQuery = "SELECT `annonce`.*,`user`.`nom`,`user`.`prenom` FROM `annonce`,`user` WHERE `annonce`.`id_user`= `user`.`id` AND ( `titre` LIKE '%$inputField%' OR `description` LIKE '%$inputField%' OR `marque` LIKE '%$inputField%' OR `modele` LIKE '%$inputField%' );";
